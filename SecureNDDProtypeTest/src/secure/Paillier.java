...
public class Paillier {
	...
	/**
	 * Construct an instance of the Paillier cryptosystem by using given public key and private key.
	 * 
	 * @param p
	 * @param q
	 * @param bitLength
	...
	public static BigInteger Enc(BigInteger m, PaillierPublicKey pk) {
		BigInteger r = new BigInteger(pk.getBitLength(), new Random());
		return pk.getG().modPow(m, pk.getNsquare()).multiply(r.modPow(pk.getN(), pk.getNsquare())).mod(pk.getNsquare());
	}
	
	public static BigInteger Dec(BigInteger c, PaillierPublicKey pk, PaillierPrivateKey sk) {

		return c.modPow(sk.getLambda(), pk.getNsquare()).subtract(BigInteger.ONE).divide(pk.getN())
				.multiply(sk.getU()).mod(pk.getN());
	}
